"""Precise Mode: YOLOv8s + BoTSORT + adaptive EMA + motion prediction.

Higher tracking accuracy for challenging underwater conditions.
Same optimisations as FastMode: GPU auto-detect, resolution scaling,
...
class PreciseModeTracker(TrackingEngine):
    def __init__(self, confidence_threshold: float = 0.6,
        ...
        # Adaptive EMA
        self._ema_alpha_min = 0.10
        self._ema_alpha_max = 0.60
    ...
    def process_frame(self, frame: np.ndarray) -> TrackingResult:
        ...
        frame_budget_ms = 1000 / 25  # Precise mode targets 25 fps
    ...
    def _real_track(self, frame: np.ndarray) -> TrackingResult:
        ...
        # Velocity update
        now = time.monotonic()
        dt = now - self._last_track_time if self._last_track_time else 0.04
        dt = max(dt, 0.001)
    ...
    def _synthetic_result(self) -> TrackingResult:
        ...
        center_x = 0.5 + 0.12 * math.sin(t * 0.4) + 0.02 * math.sin(t * 1.3)
        center_y = 0.5 + 0.08 * math.cos(t * 0.35) + 0.02 * math.cos(t * 1.1)
        bbox_w, bbox_h = 0.06, 0.14
        return TrackingResult(
            bbox=(center_x - bbox_w / 2, center_y - bbox_h / 2,
                  center_x + bbox_w / 2, center_y + bbox_h / 2),
            center_x=center_x, center_y=center_y,
            confidence=0.90 + 0.03 * math.sin(t * 0.5),

"""Precise Mode: YOLOv8s + ByteTrack + EMA smoothing.
...
class PreciseModeTracker(TrackingEngine):
    ...
    def process_frame(self, frame: np.ndarray) -> TrackingResult:
        ...
        # Adaptive skip
        frame_period_ms = 1000 / 25  # Precise mode targets 25 fps
        if self._latency_ms > frame_period_ms * 1.5:
            self._skip_frames = min(4, int(self._latency_ms / frame_period_ms) - 1)
